...
public class RangeChecker2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 画像を非表示に設定
        HideAllImages2(imageObjectsPlayer1);
        HideAllImages2(imageObjectsPlayer2);
    ...
    void Update()
    {
        ...
        bool isPlayer1InRange = IsPlayerInRange2(player1Object);//範囲内かの判定
        bool isPlayer2InRange = IsPlayerInRange2(player2Object);//範囲内かの判定

        // プレイヤー1とプレイヤー2の画像表示を統一したメソッドで処理
        HandlePlayerImageDisplay2(player1Object, isPlayer1InRange, selectedImagePlayer1, ref mainCamera, 0);
        HandlePlayerImageDisplay2(player2Object, isPlayer2InRange, selectedImagePlayer2, ref secondCamera, 1);
    }

    // 現在選ばれている問題番号に基づいて画像を更新する
    public void SetCurrentQuestionObject2(int player, GameObject questionObject)
    {
        if (player == 1)
        {
            // プレイヤー1用の選ばれた画像をセット
    ...
    // プレイヤーごとの画像表示処理
    private void HandlePlayerImageDisplay2(GameObject playerObject, bool IsPlayerInRange1, GameObject selectedImage, ref Camera camera, int displayIndex)
    {
        ...
        {
            ToggleImage2(selectedImage, ref isImageVisible);
            if (camera != null)
            {
        ...
        {
            HideAllImages2(new GameObject[] { selectedImage });
        }
    }

    // 指定されたプレイヤーが範囲内にいるか判定する
    private bool IsPlayerInRange2(GameObject playerObject)
    {
    ...
    // 画像の表示と非表示を切り替える
    private void ToggleImage2(GameObject imageObject, ref bool isVisible)
    {
        if (imageObject != null)
        {
    ...
    // すべての画像を非表示にする
    private void HideAllImages2(GameObject[] imageObjects)
    {
